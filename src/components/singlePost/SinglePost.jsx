...
export default function SinglePost() {
  ...
  useEffect(() => {
    const getPost = async () => {
      const res = await axiosInstace.get("/posts/" + path);
      setPost(res.data);
  ...
  const handleDelete = async () => {
    try {
      await axiosInstace.delete(`/posts/${post._id}`, {
  ...
  const handleUpdate = async () => {
    try {
      await axiosInstace.put(`/posts/${post._id}`, {
    ...
    <div className="singlePost">
      <div className="singlePostWrapper">
        ...
        <div className="singlePostInfo">
          ...
          <span className="singlePostDate">{new Date(post.createdAt).toLocaleDateString("id-ID")}</span>
        </div>
